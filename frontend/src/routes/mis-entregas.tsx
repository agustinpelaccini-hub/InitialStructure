...
import { useLatestPedidos, useRepartidores } from "@/hooks/apiHooks";
...
function MisEntregas() {
  ...
  const repartidor = repartidores.find((r: any) => r.id === session?.entidad_id);
  const misPedidos = pedidos.filter((p: any) => p.repartidor_id === session?.entidad_id && p.estado !== "entregado" && p.estado !== "cancelado");
  const [loading, setLoading] = useState(false);

  const handleToggleDisponible = async () => {
    if (!repartidor || !session?.entidad_id) return;
    setLoading(true);
    try {
      await api.patch(`/repartidores/${session.entidad_id}`, { disponible: !repartidor.disponible });
